...
<div class="page-container" id="box">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" @click="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe60b;</i> 批量删除</a>  <a data-href="${root}/recovery" class="btn btn-primary radius" onClick="Hui_admin_tab(this)" data-title="回收站"    href="javascript:;"><i class="Hui-iconfont">&#xe6e2;</i> 回收站</a></span> <span class="r">共有数据：<strong v-text="page.total"></strong> 条</span> </div>
	<div class="mt-20">
			<table
				class="table-sort table table-border table-bordered table-bg table-hover  table-responsive" id="table">
				<thead>
					<tr class="text-c">
						<th width="25"><input type="checkbox" id="all" name="" value="-1"></th>
						<th width="80">作者</th>
						<th>标题</th>
						<th width="80">编辑人</th>
						<th width="80">副标题</th>
				...
				<tbody id="body">
					...
					<td class="td-status"><span class="label label-warning  radius">未发布</span></td>
						
						
						<td class="f-14 td-manage">
						<a style="text-decoration:none" @Click="article_felease(article.aid,this)"
...
var vm=new Vue({		
	...
	methods:{
		getArticles:function(){
			this.$http.post("${root}/admin/article/find/drafts",{
		...
		/*文章-发布*/
		article_felease:function (id,index){
			var v=this;
			layer.confirm('确认要发布吗？',function(index1){
				$.ajax({
					url:"${root}/admin/article/felease",
					type:"post",
					...
					error:function(){
					layer.msg('发布异常!',{icon: 5,time:1000});
					}
				})
			});
		},
		...
		article_del:function (id,index){
			...
			layer.confirm('确认要删除吗？',function(index1){
				$.ajax({
					url:"${root}/admin/article/Destruction/article",
					type:"post",
					data:"Aid="+parseInt(id),
					success:function(res){
						if(res=="success"){
...
/* $('.table-sort').dataTable({
	...
	"aLengthMenu": [10, 25, 50, 100],//二组数组，第一组数量，第二组说明文字;
	"pading":true,
	"aoColumnDefs": [
